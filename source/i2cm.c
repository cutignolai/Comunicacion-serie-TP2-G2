...
#define I2C0_TEST_PIN	PORTNUM2PIN(PD, 1)
#define I2C1_TEST_PIN	PORTNUM2PIN(PB, 11)
#define I2C2_TEST_PIN	PORTNUM2PIN(PB, 20)
...
static I2C_fifo_id_t i2c_fifo[I2C_N] = {FIFO_INVALID_ID, FIFO_INVALID_ID, FIFO_INVALID_ID};
...
bool I2C_IsBusFree(uint8_t id){
	if (I2C_FIFO_IsBufferEmpty(i2c_fifo[id]) && (I2C_PTRS[id]->S & I2C_S_BUSY_MASK) != I2C_S_BUSY_MASK){
		return true;
	} else {
		return false;
	}
	...
	case I2C_READ:
		...
		if (--i2c_curr_trans[id]->count == 1){
			...
			*i2c_curr_trans[id]->ptr++ = i2c_ptr->D;		// Read data
			...
			I2C_state[id] = I2C_IDLE;
			if (!I2C_FIFO_IsBufferEmpty(i2c_fifo[id])){
				I2C_start_transaction(id);
			}
		}
		break;
...
void I2C_start_transaction(uint8_t id){
	...
	if (!i2c_curr_trans[id]->next_rsta){	// If new transaction:
		I2C_FIFO_PullFromBuffer(i2c_fifo[id], i2c_curr_trans[id]);		// Get next transaction
